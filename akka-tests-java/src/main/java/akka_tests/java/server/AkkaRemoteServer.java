...
class AkkaRemoteServer implements Bootable {
	...
	public final void setup() {
		...
		Props props =
			...
			// Props.create(GreetingActor.class);  // no error, but messages goes to deadLetters ...
			Props.create(GreetingActor.class).withDeploy(new Deploy(new RemoteScope(addr)));  // no error, but messages goes to deadLetters ...
	...
	protected final void checkSystemRemote() {
		...
		// selection.tell(new Identify(null), ACTOR_NO_SENDER);  // send a standard Identify message, so the sender actor will then receive a standard ActorIdentity response to get an ActorRef ...
